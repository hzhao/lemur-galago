...
/**
 * Tupleflow-Typebuilder automatically-generated class: IndexLink.
 */
@SuppressWarnings("unused")
public final class IndexLink implements Type<IndexLink> {
    public byte[] id;
    public int pos;
    ...
    public static final class SrctypeTargettypeIdTargetidTargetposOrder implements Order<IndexLink> {
        public int hash(IndexLink object) {
            int h = 0;
            h += CmpUtil.hash(object.srctype);
            h += CmpUtil.hash(object.targettype);
            h += CmpUtil.hash(object.id);
            h += CmpUtil.hash(object.targetid);
            h += CmpUtil.hash(object.targetpos);
            return h;
        } 
        public Comparator<IndexLink> greaterThan() {
            return new Comparator<IndexLink>() {
                public int compare(IndexLink one, IndexLink two) {
                    int result = 0;
                    do {
                        result = + CmpUtil.compare(one.srctype, two.srctype);
                        if(result != 0) break;
                        result = + CmpUtil.compare(one.targettype, two.targettype);
                        if(result != 0) break;
                        result = + CmpUtil.compare(one.id, two.id);
                        if(result != 0) break;
                        result = + CmpUtil.compare(one.targetid, two.targetid);
                        if(result != 0) break;
                        result = + CmpUtil.compare(one.targetpos, two.targetpos);
                        if(result != 0) break;
                    } while (false);
                    return -result;
                }
        ...
        public static final class OrderedWriterClass extends OrderedWriter< IndexLink > {
            IndexLink last = null;
            ...
            public void process(IndexLink object) throws IOException {
               ...
               if (processAll || last == null || 0 != CmpUtil.compare(object.srctype, last.srctype)) { processAll = true; shreddedWriter.processSrctype(object.srctype); }
               if (processAll || last == null || 0 != CmpUtil.compare(object.targettype, last.targettype)) { processAll = true; shreddedWriter.processTargettype(object.targettype); }
               if (processAll || last == null || 0 != CmpUtil.compare(object.id, last.id)) { processAll = true; shreddedWriter.processId(object.id); }
               if (processAll || last == null || 0 != CmpUtil.compare(object.targetid, last.targetid)) { processAll = true; shreddedWriter.processTargetid(object.targetid); }
               if (processAll || last == null || 0 != CmpUtil.compare(object.targetpos, last.targetpos)) { processAll = true; shreddedWriter.processTargetpos(object.targetpos); }
        ...
        public static final class ShreddedBuffer {
            ...
            public void copyUntilSrctype(ShreddedBuffer other, ShreddedProcessor output) throws IOException {
                while (!isAtEnd()) {
                    if (other != null) {   
                        ...
                        int c = + CmpUtil.compare(getSrctype(), other.getSrctype());
            ...
            public void copyUntilTargettype(ShreddedBuffer other, ShreddedProcessor output) throws IOException {
                while (!isAtEnd()) {
                    if (other != null) {   
                        assert !other.isAtEnd();
                        int c = + CmpUtil.compare(getTargettype(), other.getTargettype());
            ...
            public void copyUntilTargetid(ShreddedBuffer other, ShreddedProcessor output) throws IOException {
                while (!isAtEnd()) {
                    if (other != null) {   
                        assert !other.isAtEnd();
                        int c = + CmpUtil.compare(getTargetid(), other.getTargetid());
            ...
            public void copyUntilTargetpos(ShreddedBuffer other, ShreddedProcessor output) throws IOException {
                while (!isAtEnd()) {
                    if (other != null) {   
                        assert !other.isAtEnd();
                        int c = + CmpUtil.compare(getTargetpos(), other.getTargetpos());
        ...
        public static final class ShreddedCombiner implements ReaderSource<IndexLink>, ShreddedSource {
        ...
        public static final class ShreddedReader implements Step, Comparable<ShreddedReader>, TypeReader<IndexLink>, ShreddedSource {
            public ShreddedProcessor processor;
            ...
            public final int compareTo(ShreddedReader other) {
                ...
                int result = 0;
                do {
                    result = + CmpUtil.compare(buffer.getSrctype(), otherBuffer.getSrctype());
                    if(result != 0) break;
                    result = + CmpUtil.compare(buffer.getTargettype(), otherBuffer.getTargettype());
                    if(result != 0) break;
                    result = + CmpUtil.compare(buffer.getId(), otherBuffer.getId());
                    if(result != 0) break;
                    result = + CmpUtil.compare(buffer.getTargetid(), otherBuffer.getTargetid());
                    if(result != 0) break;
                    result = + CmpUtil.compare(buffer.getTargetpos(), otherBuffer.getTargetpos());
                    if(result != 0) break;
                } while (false);                                             
        ...
        public static final class DuplicateEliminator implements ShreddedProcessor {
            ...
            public void processSrctype(String srctype) throws IOException {  
                if (srctypeProcess || CmpUtil.compare(srctype, last.srctype) != 0) {
            ...
            public void processTargettype(String targettype) throws IOException {  
                if (targettypeProcess || CmpUtil.compare(targettype, last.targettype) != 0) {
            ...
            public void processTargetid(String targetid) throws IOException {  
                if (targetidProcess || CmpUtil.compare(targetid, last.targetid) != 0) {
            ...
            public void processTargetpos(int targetpos) throws IOException {  
                if (targetposProcess || CmpUtil.compare(targetpos, last.targetpos) != 0) {
        ...
        public static final class TupleShredder implements Processor {
            ...
            public void process(IndexLink object) throws IOException {                                                                                                                                                   
                boolean processAll = false;
                if(last == null || CmpUtil.compare(last.srctype, object.srctype) != 0 || processAll) { processor.processSrctype(object.srctype); processAll = true; }
                if(last == null || CmpUtil.compare(last.targettype, object.targettype) != 0 || processAll) { processor.processTargettype(object.targettype); processAll = true; }
                if(last == null || CmpUtil.compare(last.id, object.id) != 0 || processAll) { processor.processId(object.id); processAll = true; }
                if(last == null || CmpUtil.compare(last.targetid, object.targetid) != 0 || processAll) { processor.processTargetid(object.targetid); processAll = true; }
                if(last == null || CmpUtil.compare(last.targetpos, object.targetpos) != 0 || processAll) { processor.processTargetpos(object.targetpos); processAll = true; }
